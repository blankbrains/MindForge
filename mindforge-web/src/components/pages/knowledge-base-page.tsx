  ...
  return (
    <div className="mx-auto w-full min-w-0 max-w-5xl space-y-6">
      <div className="flex items-center justify-between gap-4">
        ...
        <button type="button" onClick={openUploadDialog} className="inline-flex shrink-0 items-center gap-2 rounded-lg bg-primary px-4 py-2.5 text-sm font-medium text-white hover:bg-primary-dark transition-colors">
      ...
      {documents.isLoading ? <LoadingSkeleton variant="card" count={4} /> : documents.isError ? (
        ...
        <EmptyState icon={<FileText className="h-12 w-12" />} title="暂无文档" description="上传 PDF、DOCX、Markdown 等文件到知识库" action={<button type="button" onClick={openUploadDialog} className="rounded-lg bg-primary px-4 py-2 text-sm font-medium text-white hover:bg-primary-dark transition-colors">上传第一篇文档</button>} />
      )}
      ...
      {uploadDialogOpen && (
        ...
        >
            ...
            <div className="mt-5 space-y-4">
              {!activeJobId && !upload.isPending && <div>
                <label htmlFor="knowledge-upload-file" className="block text-sm font-medium text-text mb-1.5">选择文件</label>
                <input id="knowledge-upload-file" ref={fileRef} type="file" accept=".pdf,.docx,.md,.txt,.html,.htm" disabled={upload.isPending} onChange={(e) => {
              ...
              {!activeJobId && !upload.isPending && <div className="flex items-center gap-5">
      ...
      {cancelConfirmOpen && (
        ...
        >
            ...
            <p id="cancel-upload-description" className="text-sm text-text-muted mb-4">
              {upload.isPending
                ? "文件尚未上传完成。确定要中止本次上传吗？"
      ...
      {viewingDoc && (
        ...
        >
            <div className="flex items-center justify-between mb-4">
              ...
              <button type="button" onClick={closeDocument} aria-label="关闭文档内容" className="rounded-lg p-1 text-text-muted hover:bg-surface-alt transition-colors"><X className="h-5 w-5" aria-hidden="true" /></button>
            </div>
