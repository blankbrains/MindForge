...
const CriticFeedbackPanel = lazy(() =>
  import("@/components/research/critic-feedback-panel").then((module) => ({
    default: module.CriticFeedbackPanel,
  })),
);
...
const StreamingAnswerPanel = lazy(() =>
  import("@/components/research/streaming-markdown").then((module) => ({
    default: module.StreamingAnswerPanel,
  })),
);
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      {!hasLLMKey && (
        <div className="flex items-center gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800 dark:border-amber-800 dark:bg-amber-950 dark:text-amber-300">
          ...
          <span>
            当前模型服务配置不完整，将使用<strong>文档检索模式</strong>。如需 AI
            分析，请先
          </span>
          <Link
            to="/settings"
      ...
      {session.isCancelled && (
        ...
        >
          ...
          <div>
            <p className="font-semibold">研究已停止</p>
            <p className="mt-1 text-xs leading-5">
              当前连接和后端研究任务已取消，未完成结果不会写入历史或记忆。
            </p>
          </div>
        </div>
      )}
      ...
      {session.isIdle && !session.finalResult && (
        <EmptyState
          ...
          title="开始新的研究"
          description="输入一个问题，Agent 将自动分解任务、检索信息、生成报告"
        />
      )}
          ...
          <div className="xl:col-span-2 space-y-6">
            ...
            {session.refineRound > 0 && (
              <div className="flex items-center gap-2 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-700 dark:border-amber-800 dark:bg-amber-950 dark:text-amber-300">
                <Loader2 className="h-4 w-4 animate-spin" /> 精炼中… 第{" "}
                {session.refineRound} 轮
              </div>
            )}
          </div>
