...
研究工作台通过 SSE 实时展示 `routing`、规划、子任务、综合、评审和精炼状态。
短且稳定的问题即使进入轻量直答，也会先显示“正在判断回答方式”，并按真实阶段更新
单任务生命周期，不会等最终答案完成后突然出现全部进度。

会话上下文由独立的 `context-store` 管理。回答完成后先刷新当前 Conversation，再加载
本轮不可变快照；上下文抽屉默认展示最新问题和回答，并可切换到只读的“本轮输入”。
当前会话支持单消息固定、轮次级遗忘和轮次级彻底删除，所有操作都有确认、加载和错误
状态；彻底删除后会清理对应快照及前端手动选择状态。
