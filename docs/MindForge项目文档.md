...
报告不使用固定六章节模板，而是根据问题复杂度自适应选择有效章节：范围集中的研究
通常使用 3-5 节，推荐、对比或多角度分析通常使用 4-7 节，深度研究通常使用
6-10 节。章节范围是软指导，信息不足时不得创建空章节或重复拆分观点。
...
- 单次模型、原生联网搜索、代码沙箱、工具排队、子任务和研究总超时可配置；
  工具执行预算不能超过子任务预算，排队预算不能超过研究总预算。
