...
**修复**：把“评审次数”和“精炼次数”拆开。先评审当前草稿，只有低于阈值且仍有
精炼预算时才重写；每次重写后进入下一次评审，因此最终报告必然对应最后一个评分。
流式链路也推送最终复评分数。`max_refine_rounds=0` 只禁止重写，复杂任务仍执行一次
评审。
...
**修复**：评估失败时返回 `evaluation_status="failed"` 和
`should_refine=False`，报告评分显示“评审失败”，不再用 `5.0` 伪装成真实分数。
合法 JSON 但缺少必需评分字段时同样按评审失败处理。
...
**现象**：知识库只命中与问题无关的片段，但融合排序第一名的归一化 RRF 分数接近
`1.0`，回退回答又把它显示成很高的质量分。

**根因**：RRF 分数只表示多个召回列表中的相对名次，不是语义相关概率；同时旧逻辑
把检索分数乘常数映射到 10 分制，并把检索相关性混同为回答质量。

**修复**：相关性优先使用归一化 CrossEncoder 分数或原始语义分数；多概念问题至少
要求命中一个有效查询概念，多个显式技术实体必须全部覆盖。检索结果只返回
`retrieval_quality`，研究链路失败时标记为 `degraded`，不再生成回答质量分。

### 77. SDK 重试与 Agent 重试叠加，失败请求耗时失控

**现象**：模型端点异常时，一次简单研究也可能持续数分钟；子任务 30 秒超时又早于
单次模型 45 秒超时，导致模型调用还未完成就被外层取消。

**根因**：OpenAI-compatible SDK 和 `BaseAgent._chat()` 同时重试，形成嵌套重试；
单次模型、子任务和研究总超时没有约束关系。

**修复**：Provider SDK 内部重试统一关闭，由 Agent 层负责有限重试；默认子任务超时
调整为 60 秒，设置 API 和前端都校验
`单次模型超时 <= 子任务超时 <= 研究总超时`。范围集中且文本较短的问题在均衡模式
直接使用单任务计划，避免慢速 Planner 调用；普通简单题跳过 Critic，短对比题、
优缺点和取舍问题仍执行 Critic，不再因只有一个子任务而漏评。

### 78. Trace 列表只显示 Orchestrator，失败原因无法追溯

**现象**：可观测列表中的每条记录都叫 `Orchestrator`，用户无法区分任务；失败研究
只显示通用错误，检索回退还会覆盖主链路失败原因。

**根因**：内部 Span 名称被直接当作用户标题，根 Trace 没有独立显示名；失败与回退
共用成功语义，历史和 Trace 保留策略也不一致。

**修复**：根 Trace 使用研究问题作为 `display_name`，内部仍保留 Orchestrator/Agent
父子链；结果区分 `success/degraded/failed/cancelled`，降级保留主链路失败原因、
Token 和费用状态。Trace 与历史默认永久保留，前端提供单条删除和清空确认。

### 79. SSE 心跳跨任务推进异步生成器导致 Trace 上下文崩溃

**现象**：多子任务研究进入流式综合后失败，错误为 `ContextVar Token was created
in a different Context`；简单问题和非流式调用不一定复现。

**根因**：心跳逻辑为每次 `__anext__()` 创建新的 `asyncio.Task`。当 Synthesizer
Span 跨越 `answer_chunk` 的 `yield` 时，Trace Token 在一个任务上下文创建，却在
下一个任务上下文退出。

**修复**：使用单一后台任务持续驱动完整研究异步生成器，通过队列向 SSE 消费端转发
事件；心跳只等待队列，不再切换生成器所属任务。回归测试覆盖多子任务、流式综合、
心跳和顶层 Trace 同时开启的路径。

### 80. 部分子任务失败仍被标记为完整成功

**现象**：多个子任务中只有一部分完成时，Synthesizer 仍能生成报告，页面显示成功和
较高报告评分，用户无法直观看出覆盖范围不完整；该结果还可能被写入长期记忆。

**根因**：最终结果只区分“全部失败”和“至少一个成功”，没有把部分失败作为独立
执行结果；记忆写入也只检查 `success`。

**修复**：部分失败统一标记为 `degraded`，结果页显示完成/失败数量与具体原因，
顶层 Trace 同步标记为降级；不完整报告不进入情节记忆和语义记忆。

### 81. 联网搜索状态只显示“未配置”，无法说明隐私和降级行为

**现象**：设置页显示 `Tavily 未配置`，用户无法判断是系统故障、缺少 API Key，
还是仍会自动使用其他外部搜索服务。

**根因**：状态只展示布尔值；工具内部虽然支持 DuckDuckGo 回退，但 Orchestrator
只有在用户配置 `TAVILY_API_KEY` 后才注册联网工具，两层语义没有在页面和文档中说明。

**修复**：Tavily 保留为显式可选后端。未配置 Key 时明确显示联网搜索未启用，不把
研究查询发送给第三方；配置后优先使用 Tavily，请求失败时才回退 DuckDuckGo。
重排序状态同步区分正常、等待加载和加载失败，研究模式选中态增加高对比背景、描边、
勾选标记和 `aria-pressed`。

### 82. SSE Trace 跨上下文退出并覆盖真实失败原因

**现象**：研究失败后，顶部只显示 `ContextVar ... was created in a different
Context`；具体 LLM 节点只有“已取消”状态，无法判断是模型接口失败、请求超时，
还是上层子任务超时。

**根因**：SSE 响应生成器可能由不同异步上下文继续或关闭，顶层 Trace 的
`ContextVar` Token 因此在错误上下文重置；同时空字符串 `TimeoutError`、子任务超时
和被动取消没有结构化错误码，前端也没有渲染观察节点的 `error` 字段。

**修复**：使用单一后台生产任务完整持有 SSE Trace，上层响应只消费字节队列；追踪
上下文重置失败仅记录系统错误，不再中断研究。LLM、子任务和研究阶段统一记录错误码、
类型、阶段、Agent、模型、尝试次数和超时值；详情页按节点展示，顶部只汇总主因，
被上层超时连带取消的 LLM 调用作为后果保留在链路中。
...
当前编号到 **82**，每项都落地到代码或自动化测试。面试官问到的时候，
