...
- API 前缀为 `/api/v1`，覆盖研究、文档、设置、历史和可观测接口。
...
- 提交前运行完整 pytest、前端回归测试、ESLint、生产构建和 Compose 配置校验。
...
- 数据库通过 Alembic 迁移，研究历史已包含 Trace ID 关联字段。
...
- 文档卡片只展示实际成功应用的 RAPTOR/GraphRAG；历史详情和研究结果使用同一
  Markdown/代码高亮渲染链路。
- 研究完成后展示 Token 与估算费用状态；GFM 表格有边框并在窄屏内独立滚动，
  成功任务清空输入，失败任务保留问题便于重试。
- 可观测列表和详情以研究问题为标题，内部保留 Orchestrator/Agent/LLM/工具父子链；
  失败与检索降级保留原因，Trace 和历史默认永久保留且支持手动删除。
- 设置页可配置研究模式、来源策略、相关性阈值、超时预算和 Langfuse；保存结果写入
  服务端 `.env`，密钥重新读取时只返回掩码。
