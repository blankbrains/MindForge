#!/usr/bin/env python3
"""Generate professional test documents using the configured LLM provider."""
...
async def gen_doc(filename, title, prompt):
    ...
    result = await llm.chat(
        [
            ChatMessage(
                role="system",
                content="你是一个AI技术文档专家。用中文生成专业、详细、结构化的技术文档，包含标题、章节、代码示例和引用。",
            ),
...
async def main():
    topics = [
        ...
        (
            "graphrag_and_raptor.md",
            "GraphRAG 与 RAPTOR：层次化与图结构检索",
