...
8. 按需要配置其余页签：
   - “检索”：设置 Top-K、语义相关性阈值和关键词覆盖阈值。重排序状态会区分
     ...
     可调整模型原生联网搜索超时；Tavily、DuckDuckGo 都不是模型 API 正常回答
     的前置条件。
   - “研究流程”：选择快速/均衡/深度模式、来源策略和回退开关；超时需满足
     `单次模型调用/原生联网搜索/代码执行 <= 子任务 <= 研究总超时`，
     工具排队超时不能超过研究总超时。
...
- **自适应报告深度** — 普通研究、推荐对比和深度研究使用不同内容预算；
  Synthesizer 按问题复杂度选择有效章节，不固定六章节，不创建空章节或重复内容
...
- **研究模式与预算** — 快速、均衡、深度三种模式；简单问题跳过不必要的规划与评审，
  模型、联网搜索、代码执行、子任务、排队和研究总超时可配置并按所属预算校验
