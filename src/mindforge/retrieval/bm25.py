...
class BM25Retriever:
    ...
    def replace_document(
        ...
        """Replace all sparse chunks for one document in a single rebuild."""
        if not documents:
            return
        with self._lock:
            kept = []
            ...
            replacements = [
                {**document, "id": str(document.get("id", index))}
                for index, document in enumerate(documents)
            ]
            self.build_index([*kept, *replacements])
    ...
    def save(self, path: Optional[str] = None) -> None:
        ...
        with self._lock:
            meta = {
                ...
                "metadatas": [dict(metadata) for metadata in self.metadatas],
            }
